...
public class template : MonoBehaviour {
	...
	void Update () {
        ...
        audioPegel = 2.0f*rmsValue;
        audioPegel = Mathf.Min(audioPegel, 1.0f);
        ...
        pitchInHz = pitchDetector.lastFrequency(); ;
        pitchInCent = 0.0f;

        if (pitchInHz != 0)
        {
            cent = 6900+1200*Mathf.Log(pitchInHz/440,2);
            m_achtelnote.setCents(cent);
        }
